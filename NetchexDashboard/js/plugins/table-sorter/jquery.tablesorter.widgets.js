	...
	format: function(table) {
		...
		win
			.scroll(function(){
				...
				sticky.css({
					left : offset.left -  (win.scrollLeft() - 1) ,
					visibility : vis
				});
				if (vis !== laststate) {
			...
			.resize(function(){
				var ht = 0;
				sticky.css({
					left : firstCell.offset().left - (win.scrollLeft() - 1) ,
					width: (header.outerWidth() - 1)
				}).each(function(i){
					$(this).css('top', ht);
...
$.tablesorter.addWidget({
    ...
    // added new widget called repeatHeaders cwf 
    // ************************************ 
    $.tablesorter.addWidget({
